import { r as createServerFn, s as getRequestHost$1 } from "./server-PdCEgQXm.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/facebook.functions-BgvbTgzh.js
var FB_SCOPES = [
	"pages_messaging",
...
var getWebhookConfig = createServerFn({ method: "GET" }).middleware([requireWorkspaceAuth]).handler(getWebhookConfig_createServerFn_handler, async ({ context }) => {
	const { resolveFacebookApp, ensureCentralVerifyToken } = await import("./facebook-app.server-CJ5Qw0y6.mjs");
	const app = await resolveFacebookApp(context.userId);
