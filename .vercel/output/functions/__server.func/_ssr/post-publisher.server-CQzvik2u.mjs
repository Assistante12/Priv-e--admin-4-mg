...
import { generateAiReply, sanitizeAiResponse, sanitizeReply } from "./ai-engine.server-CTjOATz3.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/post-publisher.server-CQzvik2u.js
var IMAGE_ENHANCE_MODEL = "google/gemini-2.5-flash-image";
var IMAGE_VARIATION_PROMPTS = [
...
async function downloadStorageImage(path) {
	...
	return {
		...
		base64: btoa(Array.from(buf, (b) => String.fromCharCode(b)).join("")),
		bytes: buf
	};
}
/** Enhance an image via Lovable AI Gemini image model. Returns null on failure. */
async function enhanceImage(mime, base64) {
...
async function generatePostDescription(opts) {
	const { data: customPrompt } = await supabaseAdmin.from("prompts").select("content").eq("user_id", opts.userId).eq("category", "post").eq("is_active", true).order("updated_at", { ascending: false }).limit(1).maybeSingle();
	const systemPrompt = customPrompt?.content?.trim() || "Ianao dia mpanao publicité manam-pahaizana amin'ny Facebook. Mamoaka description manintona, professionnel, mahatoky sy tena mandresy lahatra ny olona hametraka commentaire, J'aime sy handefa message privée. Manoratra amin'ny fiteny mifanaraka amin'ny lohateny (malagasy/frantsay/anglisy). Ampiasao emoji mifanaraka fa aza be loatra. Ampio call-to-action mazava amin'ny farany (ohatra: Commentez, Likez, Envoyez un message). Aza mampiasa markdown *, #, ** — soraty mivantana ny lahatsoratra. Aza mametraka lien externe. ZAVATRA TSY MAINTSY TANDREMINA: TSY MAMETRAKA MIHITSY resaka fandinihana (thinking), drafitra, na teny Anglisy fanazavana (toy ny Hook, Value Proposition, Check, Self-Correction). Manomboka mivantana amin'ny lahatsoratra tokony havoaka izy io, tsy misy drafi-panoratana akory. Halavany: 4 ka hatramin'ny 8 andalana, misy fiatoana mahafinaritra.";
	const hint = opts.userHint?.trim();
	const parts = [{ text: `Lohatenin'ny publication: "${opts.title}"\n\n` + (hint ? `Toromarika manokana avy amin'ny tompon'ny page (arahina tsara): ${hint}\n\n` : "") + `Amoahy description tsara sy manintona ho amin'ity publication ity.` }];
	if (opts.image) parts.push({ inline_data: {
		mime_type: opts.image.mime,
	...
	return {
		text: sanitizeReply(cleaned, false),
...
async function publishToFacebook(opts) {
	const urls = (opts.imageUrls?.length ? opts.imageUrls : opts.imageUrl ? [opts.imageUrl] : []).filter(Boolean);
	if (urls.length === 1) {
		...
		if (!res.ok || !json.id) throw new Error(`Facebook photo POST échoué: ${json?.error?.message ?? res.status}`);
	...
	if (urls.length > 1) {
		...
		const json = await res.json();
		if (!res.ok || !json.id) throw new Error(`Facebook album POST échoué: ${json?.error?.message ?? res.status}`);
	...
	const json = await res.json();
	if (!res.ok || !json.id) throw new Error(`Facebook feed POST échoué: ${json?.error?.message ?? res.status}`);
	return { post_id: json.id };
}
...
async function publishVideoToFacebook(opts) {
	...
	const json = await res.json();
	if (!res.ok || !(json.id || json.post_id)) throw new Error(`Facebook video POST échoué: ${json?.error?.message ?? res.status}`);
	return { post_id: json.post_id ?? json.id };
}
/** Run the full publish pipeline for one scheduled post row. */
async function runScheduledPost(postId) {
	const { data: post, error } = await supabaseAdmin.from("scheduled_posts").select("*").eq("id", postId).maybeSingle();
	if (error || !post) return {
		ok: false,
		error: error?.message ?? "Publication tsy hita"
	};
	...
	try {
		let pageRow = null;
		if (post.page_id) {
			const { data } = await supabaseAdmin.from("facebook_pages").select("*").eq("id", post.page_id).maybeSingle();
			pageRow = data;
		}
		if (!pageRow) {
			const { data } = await supabaseAdmin.from("facebook_pages").select("*").eq("user_id", post.user_id).eq("is_connected", true).order("created_at", { ascending: true }).limit(1).maybeSingle();
			pageRow = data;
		}
		if (!pageRow) throw new Error("Tsy misy pejy Facebook mifandray. Ampifandraiso aloha ao amin'ny pejy Facebook.");
		const rawPaths = (() => {
			const many = post.image_paths ?? [];
			if (many.length) return many;
			return post.image_path ? [post.image_path] : [];
		})();
		let firstImage = null;
		const publishPaths = [];
		if (!post.video_path && rawPaths.length) for (const p of rawPaths) {
			let usePath = p;
			try {
				...
				if (post.enhance_image) {
					...
					if (enhanced) {
						...
						const { error: upErr } = await supabaseAdmin.storage.from("post-images").upload(outPath, bin, {
							contentType: enhanced.mime,
							upsert: true
						});
		...
		if (post.video_path) {
			let fileUrl;
			if (/^https?:\/\//i.test(post.video_path)) fileUrl = post.video_path;
			else {
				const { data: signed, error: sErr } = await supabaseAdmin.storage.from("post-videos").createSignedUrl(post.video_path, 21600);
				if (sErr || !signed?.signedUrl) throw new Error(`Video URL indisponible: ${sErr?.message ?? "unknown"}`);
				fileUrl = signed.signedUrl;
			}
			fbPostId = (await publishVideoToFacebook({
			...
			const imageUrls = [];
			for (const p of publishPaths) if (p.startsWith("http://") || p.startsWith("https://")) imageUrls.push(p);
				...
				if (parsed) {
					...
					await supabaseAdmin.storage.from("post-images").upload(outPath, bin, {
						contentType: parsed.mime,
						upsert: true
					});
					const { data: s } = await supabaseAdmin.storage.from("post-images").createSignedUrl(outPath, 3600);
					if (s?.signedUrl) imageUrls.push(s.signedUrl);
				}
			} else {
				const { data: signed, error: sErr } = await supabaseAdmin.storage.from("post-images").createSignedUrl(p, 3600);
				if (sErr || !signed?.signedUrl) throw new Error(`Image URL indisponible: ${sErr?.message ?? "unknown"}`);
